# this file is taken from https://github.com/bytetriper/RAE
...
class SelfAttentionNoDrop(nn.Module):
    def __init__(
        ...
        self.qkv, self.proj = (
            nn.Linear(embed_dim, embed_dim * 3, bias=True),
...
class BatchNormLocal(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        G = np.ceil(
            x.size(0) / self.virtual_bs
        ).astype(
            int
        )  # disbale ghostnorm , for there will be division problem when bs % virtual_bs != 0
